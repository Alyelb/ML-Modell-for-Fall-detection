...
Um die Wirksamkeit dieses Vorgehens bereits vor Verfügbarkeit der
eigenen Hardware zu überprüfen, wurde der UMAFall-Knöcheldatensatz
\cite{umafall2017} als Zwischenschritt herangezogen; die Ergebnisse
sind in Abschnitt~\ref{sec:crossposition} dargestellt.

\section{Cross-Position-Transfer: Quantifizierung der Domänenlücke}
\label{sec:crossposition}

\subsection{Motivation und Versuchsaufbau}

Die in Abschnitt~\ref{sec:domain} beschriebene Domänenlücke wurde
bislang lediglich argumentativ begründet. Um sie \emph{quantitativ} zu
erfassen, wurde ein Transferexperiment durchgeführt. Als Zielposition
dient der Knöchelsensor des UMAFall-Datensatzes \cite{umafall2017},
da dieser unter allen öffentlich verfügbaren Sturzdatensätzen die
größte Nähe zur geplanten Sohlenposition aufweist. Ein
fußsohlenbasierter öffentlicher Datensatz existiert nach derzeitigem
Kenntnisstand nicht; diese Lücke in der Literatur ist zugleich ein
wesentliches Alleinstellungsmerkmal des vorliegenden Projekts.

UMAFall verwendet fünf gleichzeitig getragene Sensoren, deren
Zuordnung (Sensor-ID zu Körperposition) im Dateikopf jeder Aufnahme
dokumentiert ist. Diese Zuordnung ist \emph{nicht} über alle Dateien
identisch, weshalb sie bei der Auswertung dateiweise ausgelesen wird.
Aus den Aufzeichnungen wurden ausschließlich die Kanäle des
Knöchelsensors extrahiert, Beschleunigungs- und Gyroskopströme
zeitlich synchronisiert und von der nativen Abtastrate ($20$\,Hz) auf
die Eingangsrate des Modells ($50$\,Hz) hochgetastet. Die Fensterung
erfolgte analog zur SisFall-Pipeline. Insgesamt standen
6800~Fenster von 19~Probanden zur Verfügung (471~Stürze, 6329~ADL).

Ergänzend wurde derselbe Transfer auf KFall \cite{kfall2021}
angewendet (unterer Rücken, $100$\,Hz, 32~Probanden, 15~Sturzarten,
5075~Aufnahmen). KFall dient dabei als Gegenprobe: Da sich dieser
Datensatz in Probandenkollektiv, Sensorhardware und Protokoll
ebenso deutlich von SisFall unterscheidet wie UMAFall, jedoch eine
deutlich höhere Aufnahmequalität aufweist, lässt sich unterscheiden,
ob ein beobachteter Leistungsabfall dem Datensatzwechsel an sich oder
den spezifischen Eigenschaften eines Datensatzes zuzuschreiben ist.

Bei der Aufbereitung von KFall wurde die Zuordnung der Aufgaben-IDs
zu den Klassen (T01--T21 = ADL, T22--T36 = Sturz) nicht aus der
Dokumentation übernommen, sondern empirisch überprüft: Der Median der
Spitzenbeschleunigung steigt exakt ab T22 sprunghaft an
($1{,}84$\,g gegenüber $4{,}62$\,g), was die angenommene Grenze
bestätigt. Bemerkenswert ist dabei eine deutliche Überlappung der
Wertebereiche -- einzelne ADL-Aufgaben (T20: $4{,}49$\,g) erreichen
höhere Spitzenwerte als einzelne Sturzarten (T35: $2{,}23$\,g). Dies
belegt unmittelbar, weshalb rein schwellwertbasierte Verfahren keine
hohe Spezifität erreichen können, und begründet den Einsatz von
Modellen, die den gesamten zeitlichen Verlauf auswerten.

Es wurden zwei Bedingungen verglichen:

\begin{itemize}
\item \textbf{Zero-Shot:} Das unverändert auf SisFall (Hüfte)
      trainierte 1D-CNN wird ohne jede Anpassung auf die
      Knöcheldaten angewendet.
\item \textbf{Fine-Tuning:} Die vorderen Faltungsblöcke werden
      eingefroren, die übrigen Schichten mit kleiner Lernrate
      ($10^{-4}$) auf einem Teil der Knöcheldaten nachtrainiert.
      Die Bewertung erfolgt subjektunabhängig auf demselben
      Testsplit wie im Zero-Shot-Fall, sodass beide Bedingungen
      direkt vergleichbar sind.
\end{itemize}

\subsection{Ergebnisse}

Tabelle~\ref{tab:crossposition} und Abbildung~\ref{fig:crossposition}
fassen die Ergebnisse zusammen.

\begin{table}[H]
\centering
\caption{Ergebnisse aller Transferexperimente. Alle Werte auf
         subjektunabhängigen Testsplits; Fine-Tuning mit 60~Epochen.}
\label{tab:crossposition}
\small
\begin{tabularx}{\textwidth}{l l l C{1.7cm} C{1.7cm} C{1.7cm}}
\toprule
\textbf{Datensatz} & \textbf{Position} & \textbf{Bedingung} & \textbf{Sensit.} & \textbf{Spezif.} & \textbf{Genauigk.} \\
\midrule
...
\midrule
FallAllD                   & Taille   & Zero-Shot  & 13,6\,\% & 98,8\,\% & 97,5\,\% \\
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{Fotos/Cross_position_transfer_Ankle.png}
  \caption{Sensitivität und Spezifität des SisFall-Modells auf den
           UMAFall-Knöcheldaten vor und nach dem Fine-Tuning.}
  \label{fig:crossposition}
\end{figure}

\subsection{Kreuzvalidierung auf FallAllD}
\label{sec:crossfallalld}

Als dritter externer Datensatz wurde FallAllD~\cite{fallalld2020}
herangezogen. Dieser zeichnet sich durch eine hohe native Abtastrate
von $238$\,Hz, 35~Sturztypen und drei simultane Sensorpositionen
(Nacken, Handgelenk, Taille) aus. Für den Transferversuch wurde
ausschließlich der Taillensensor (Device~3) verwendet, da diese
Position der Hüftposition von SisFall am nächsten kommt. Die
Rohdaten liegen als separate Dateien für Beschleunigung und
Drehrate vor (LSM9DS1-Sensor, Umrechnung: $\text{ACC} = \text{raw}
\times 0{,}000244$\,g, $\text{GYR} = \text{raw} \times 0{,}07$\,${}^\circ$/s).

Verglichen mit den vorangegangenen Transferversuchen liefert
FallAllD ein überraschendes Ergebnis: Obwohl die Abtastrate
($238$\,Hz) höher ist als bei jedem anderen hier betrachteten
Datensatz, fällt die Zero-Shot-Sensitivität auf lediglich
$13{,}6\,\%$ -- deutlich unter dem KFall-Ergebnis ($83{,}5\,\%$)
und sogar unter dem UMAFall-Ergebnis ($42{,}2\,\%$). Fine-Tuning
hebt die Sensitivität auf $84{,}6\,\%$ bei einer Spezifität von
$98{,}7\,\%$, was zeigt, dass die Daten ausreichend Information
für die Sturzerkennung enthalten.

\subsubsection{Kontrollexperiment: Clipping bei $\pm$8\,g}
\label{sec:clipping}

Der LSM9DS1-Sensor in FallAllD misst mit einem Bereich von
$\pm 8$\,g, während SisFall ($\pm 16$\,g) und KFall ($\pm 16$\,g)
den doppelten Bereich abdecken. Um zu prüfen, ob das
\emph{Clipping} hoher Beschleunigungsspitzen durch den
eingeschränkten Messbereich den Zero-Shot-Einbruch verursacht,
wurde ein Kontrollexperiment durchgeführt: Die SisFall-Testdaten
wurden synthetisch bei $\pm 8$\,g, $\pm 4$\,g und $\pm 2$\,g
begrenzt und erneut evaluiert.

\begin{table}[H]
\centering
\caption{Clipping-Experiment: Sensitivität des SisFall-Modells bei
         synthetischer Begrenzung des Beschleunigungsbereichs.}
\label{tab:clipping}
\small
\begin{tabularx}{0.75\textwidth}{l C{1.7cm} C{1.7cm} C{1.7cm}}
\toprule
\textbf{Bereich} & \textbf{Sensit.} & \textbf{Spezif.} & \textbf{Genauigk.} \\
\midrule
$\pm 16$\,g (Original) & 98,7\,\% & 99,0\,\% & 99,0\,\% \\
$\pm\;\;8$\,g          & 98,7\,\% & 99,0\,\% & 99,0\,\% \\
$\pm\;\;4$\,g          & 98,3\,\% & 99,3\,\% & 99,3\,\% \\
$\pm\;\;2$\,g          & 98,0\,\% & 99,9\,\% & 99,8\,\% \\
\bottomrule
\end{tabularx}
\end{table}

Das Ergebnis ist eindeutig: Die Begrenzung auf $\pm 8$\,g verändert
die Klassifikationsleistung nicht, da nur $0{,}5\,\%$ aller
SisFall-Fenster überhaupt Spitzenwerte oberhalb von $8$\,g aufweisen.
Selbst bei $\pm 2$\,g sinkt die Sensitivität lediglich um
$0{,}7$~Prozentpunkte. Der eingeschränkte Messbereich des LSM9DS1
scheidet damit als Ursache für den Zero-Shot-Einbruch aus.

\subsubsection{Ursachenanalyse}

Die Kombination aus hoher Abtastrate, ausreichendem Messbereich
\emph{und} dennoch niedrigem Zero-Shot-Ergebnis legt nahe, dass
die sensorspezifischen Signalcharakteristiken den Transfererfolg
maßgeblich beeinflussen. Unterschiedliche Sensoren erzeugen
unterschiedliche Signalformen desselben physikalischen Ereignisses,
bedingt durch Unterschiede in der internen Filterung, der
Rauschcharakteristik und der Ansprechzeit. Das CNN erlernt diese
hardwarespezifischen Muster, die sich beim Wechsel der Hardware
nicht übertragen. Der Erfolg des Fine-Tunings ($84{,}6\,\%$)
bestätigt, dass die relevanten physikalischen Informationen
vorhanden sind -- sie sind lediglich in einer Form kodiert, die
das auf ADXL345/ITG3200-Daten trainierte Modell nicht unmittelbar
erkennt.

Tabelle~\ref{tab:domainfactors} fasst die Einflussfaktoren über
alle drei externen Datensätze zusammen.

\begin{table}[H]
\centering
\caption{Einflussfaktoren auf den Zero-Shot-Transfer. Der Erfolg
         hängt nicht von einem einzelnen Faktor ab, sondern von
         der Gesamtheit der Domänenunterschiede.}
\label{tab:domainfactors}
\small
\begin{tabularx}{\textwidth}{l C{1.3cm} C{1.3cm} l C{1.3cm} C{1.3cm}}
\toprule
\textbf{Datensatz} & \textbf{Hz} & \textbf{Bereich} & \textbf{Sensor} & \textbf{ZS-Se} & \textbf{FT-Se} \\
\midrule
KFall    & 100 & $\pm 16$\,g & Xsens MTw    & 83,5\,\% & 96,4\,\% \\
UMAFall  &  20 & $\pm 16$\,g & SensorTag    & 42,2\,\% & 92,2\,\% \\
FallAllD & 238 & $\pm\;\;8$\,g & LSM9DS1   & 13,6\,\% & 84,6\,\% \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Falsch-Positiv-Test auf fußnahen Alltagsdaten}
\label{sec:fptest}

Alle bisherigen Transferversuche bewerten die Sensitivität, also die
Fähigkeit, Stürze zu erkennen. Für den praktischen Einsatz ist jedoch
die Falsch-Positiv-Rate mindestens ebenso entscheidend: Ein System,
das bei jedem zweiten Schritt Alarm auslöst, ist unbrauchbar,
unabhängig von seiner Sensitivität.

\subsubsection{Vorbemerkung zur Smart Insole Database}

Ursprünglich war für diesen Test die Smart Insole
Database (Datensatz \texttt{tg0uk/Smart\_Insole\_Database})
vorgesehen, da sie als einzige öffentliche Quelle Bewegungsdaten
direkt aus einer Schuhsohle enthält. Die Prüfung der veröffentlichten Dateien ergab jedoch, dass
diese entgegen der Angabe in der Dokumentation \emph{keine}
IMU-Kanäle enthalten. Jede der 30~Dateien (fünf Probanden $\times$
sechs Aktivitäten) umfasst ausschließlich 38~CapSense-Druckelektroden
sowie die Bodenreaktionskräfte der Kraftmessplatte
($F_x$, $F_y$, $F_z$). Beschleunigungs- und Drehratenkanäle fehlen
vollständig. Der Datensatz ist damit für die vorliegende
Fragestellung nicht verwendbar.

Dies ist der vierte im Projektverlauf dokumentierte Fall, in dem eine
Quellenangabe von den tatsächlichen Daten abweicht, und bestätigt das
methodische Prinzip, Spezifikationen grundsätzlich gegen die
Primärdaten zu prüfen.

Als nächstbeste fußnahe Quelle wurden daher die
UMAFall-Knöcheldaten herangezogen, beschränkt auf ADL-Aufzeichnungen.

\subsubsection{Versuchsaufbau und Ergebnis}

Das auf SisFall trainierte CNN wurde ohne Anpassung auf sämtliche
ADL-Aufzeichnungen der UMAFall-Knöchelposition angewendet
($6329$~Fenster, entsprechend rund $1{,}76$~Stunden
Aufzeichnungszeit). Da keine Stürze enthalten sind, ist jede positive
Klassifikation per Definition ein Fehlalarm.

\begin{table}[H]
\centering
\caption{Falsch-Positiv-Rate nach Aktivität (Entscheidungsschwelle
         $0{,}5$), UMAFall-Knöchelposition, ausschließlich
         ADL-Aufzeichnungen.}
\label{tab:fptest}
\small
\begin{tabularx}{0.9\textwidth}{l C{1.8cm} C{1.3cm} C{1.6cm} C{1.5cm}}
\toprule
\textbf{Aktivität} & \textbf{Fenster} & \textbf{FP} & \textbf{Rate} & \textbf{max. $p$} \\
\midrule
Joggen                  & 259 & 46 & 17,76\,\% & 1,000 \\
Tür öffnen              & 563 & 32 &  5,68\,\% & 1,000 \\
Gehen                   & 713 & 38 &  5,33\,\% & 1,000 \\
Treppe abwärts          & 259 & 12 &  4,63\,\% & 1,000 \\
Treppe aufwärts         & 255 &  8 &  3,14\,\% & 1,000 \\
Hinlegen (Bett)         & 624 & 18 &  2,88\,\% & 1,000 \\
Arme heben              & 567 & 13 &  2,29\,\% & 1,000 \\
Telefonieren            & 610 & 11 &  1,80\,\% & 1,000 \\
Sitzen/Aufstehen        & 699 & 11 &  1,57\,\% & 1,000 \\
Hüpfen                  & 584 &  8 &  1,37\,\% & 0,996 \\
Bücken                  & 648 &  8 &  1,23\,\% & 1,000 \\
Klatschen               & 548 &  3 &  0,55\,\% & 0,996 \\
\midrule
\textbf{Gesamt}         & \textbf{6329} & \textbf{208} & \textbf{3,29\,\%} & --- \\
\bottomrule
\end{tabularx}
\end{table}

Bei einer Gesamtrate von $3{,}29\,\%$ entspricht dies hochgerechnet
rund $118$~Fehlalarmen pro Stunde ununterbrochener Aufzeichnung. Ein
solches System wäre im Alltag nicht einsetzbar.

Aufschlussreich ist die Verteilung: Die höchsten Raten treten bei
\emph{Joggen} ($17{,}76\,\%$), \emph{Gehen} ($5{,}33\,\%$) und
\emph{Treppensteigen} auf -- also genau bei jenen Aktivitäten, bei
denen der Fuß wiederholt auf den Boden aufschlägt. Der Fersenauftritt
erzeugt am Knöchel einen kurzen, steilen Beschleunigungsimpuls, der
dem Aufprallereignis eines Sturzes an der Hüfte formal ähnelt. Das
Modell erkennt dieses Muster korrekt als Impact -- die Zuordnung zur
Klasse \glqq Sturz\grqq{} ist jedoch im fußnahen Kontext falsch.

Bemerkenswert ist ferner, dass die maximale Klassenwahrscheinlichkeit
in fast allen Aktivitäten bei $1{,}000$ liegt. Das Modell ist sich
seiner Fehlklassifikation also nicht etwa unsicher, sondern maximal
zuversichtlich. Eine bloße Anhebung der Entscheidungsschwelle löst
das Problem daher nicht vollständig.

\subsection{Progressives Fine-Tuning}
\label{sec:progressive}

Die vorangegangenen Experimente zeigen, dass Fine-Tuning die
Domänenlücke zuverlässig schließt. Offen bleibt, ob die
\emph{Reihenfolge} der Anpassungsschritte eine Rolle spielt. Für den
Einsatz in der Insole ist dies unmittelbar relevant: Sollte das
Modell direkt von SisFall auf die Fußdaten angepasst werden, oder
empfiehlt sich ein Zwischenschritt über einen weiteren
Sturzdatensatz?

Verglichen wurden drei Strategien auf einem \emph{identischen},
subjektunabhängigen Testsatz der UMAFall-Knöcheldaten:

\begin{table}[H]
\centering
\caption{Vergleich der Transferstrategien auf identischem Testsatz
         (UMAFall, Knöchel). Alle Werte subjektunabhängig.}
\label{tab:progressive}
\small
\begin{tabularx}{\textwidth}{l X C{1.6cm} C{1.6cm} C{1.6cm}}
\toprule
& \textbf{Strategie} & \textbf{Sensit.} & \textbf{Spezif.} & \textbf{Genauigk.} \\
\midrule
A & Zero-Shot (SisFall)                & 30,6\,\% & 96,0\,\% & 94,8\,\% \\
B & Direkt: SisFall $\rightarrow$ UMAFall & 33,3\,\% & 95,2\,\% & 94,1\,\% \\
C1 & Zwischenstand nach KFall-Stufe    & 22,2\,\% & 96,0\,\% & 94,7\,\% \\
\textbf{C2} & \textbf{Progressiv: SisFall $\rightarrow$ KFall $\rightarrow$ UMAFall} & \textbf{75,0\,\%} & \textbf{97,8\,\%} & \textbf{97,4\,\%} \\
\bottomrule
\end{tabularx}
\end{table}

Das progressive Vorgehen übertrifft die direkte Anpassung um
$41{,}7$~Prozentpunkte Sensitivität, bei gleichzeitig \emph{höherer}
Spezifität ($97{,}8\,\%$ gegenüber $95{,}2\,\%$).

Besonders aufschlussreich ist der Zwischenstand C1: Nach der
KFall-Stufe allein \emph{sinkt} die Sensitivität auf den
UMAFall-Knöcheldaten von $30{,}6\,\%$ auf $22{,}2\,\%$. Die
Zwischenstufe verbessert die unmittelbare Leistung also nicht --
sie verschlechtert sie sogar. Erst der darauffolgende
Anpassungsschritt profitiert erheblich von ihr.

Eine plausible Erklärung liefert die Betrachtung der gelernten
Merkmalsrepräsentation: Ein ausschließlich auf SisFall trainiertes
Modell kodiert Stürze in der spezifischen Signalform des
ADXL345/ITG3200. Die KFall-Stufe konfrontiert es mit einer zweiten,
davon abweichenden Ausprägung desselben physikalischen Ereignisses.
Die resultierende Repräsentation ist allgemeiner und damit leichter
an eine dritte Domäne anpassbar, auch wenn sie für keine der beiden
Ausgangsdomänen optimal ist.

\medskip
\noindent\textit{Anmerkung:} Die Absolutwerte in
Tabelle~\ref{tab:progressive} weichen von denen in
Tabelle~\ref{tab:crossposition} ab, da hier ein anderer
Datenaufteilungs-Seed und ein einheitliches Fine-Tuning-Protokoll für
alle drei Strategien verwendet wurde. Für den hier angestellten
Vergleich ist dies unerheblich, da alle drei Strategien auf demselben
Testsatz und mit identischen Hyperparametern bewertet wurden.

\subsection{Zeitliche Plausibilisierung durch N-von-M-Voting}
\label{sec:voting}

Der Falsch-Positiv-Test (Abschnitt~\ref{sec:fptest}) zeigt, dass eine
Entscheidung auf Basis eines einzelnen Fensters für den praktischen
Einsatz ungeeignet ist. Naheliegend ist daher eine zeitliche
Plausibilisierung: Ein Alarm wird erst ausgelöst, wenn innerhalb der
letzten $M$ Entscheidungen mindestens $N$ positiv ausfallen. Die
zugrundeliegende Annahme lautet, dass ein Sturz aufgrund seiner Dauer
mehrere aufeinanderfolgende positive Fenster erzeugt, ein einzelner
Fersenauftritt hingegen nur ein isoliertes.

\subsubsection{Einfluss der Fensterschrittweite}

Ein erster Versuch mit der bislang verwendeten Schrittweite von
$1{,}0$\,s ($50\,\%$ Überlappung) ließ die Sensitivität bereits bei
der Regel \emph{3-von-3} auf null fallen. Die Ursache liegt in der
Fensterung selbst: Bei einer Aufprallphase von $50$--$100$\,ms und
einer Schrittweite von $1{,}0$\,s liegt das Aufprallereignis in nur
einem einzigen Fenster. Aufeinanderfolgende positive Fenster können
konstruktionsbedingt gar nicht entstehen, sodass jede Regel mit
$N \geq 2$ zwangsläufig scheitern muss.

Die Schrittweite wurde daher auf $0{,}2$\,s ($90\,\%$ Überlappung)
reduziert. Das Aufprallereignis erscheint dann in bis zu zehn
aufeinanderfolgenden Fenstern, wobei seine relative Position innerhalb
des Fensters schrittweise wandert. Da das Modell auf
impact-zentrierten Fenstern trainiert wurde, reagiert es am
deutlichsten, wenn der Aufprall nahe der Fenstermitte liegt.

\subsubsection{Ergebnis}

Tabelle~\ref{tab:voting} stellt die Regeln für das unveränderte
SisFall-Modell und für das auf Knöcheldaten feinabgestimmte Modell
gegenüber (Entscheidungsschwelle jeweils $0{,}9$, subjektunabhängiger
Testsatz).

\begin{table}[H]
\centering
\caption{N-von-M-Voting bei $0{,}2$\,s Schrittweite,
         Entscheidungsschwelle $0{,}9$. Se bezeichnet den Anteil der
         Sturzaufzeichnungen, in denen mindestens einmal Alarm
         ausgelöst wurde; FP/Std die Fehlalarme pro Stunde
         ADL-Aufzeichnung.}
\label{tab:voting}
\small
\begin{tabularx}{\textwidth}{l C{1.6cm} C{1.7cm} C{1.6cm} C{1.7cm}}
\toprule
& \multicolumn{2}{c}{\textbf{Basismodell}} & \multicolumn{2}{c}{\textbf{Feinabgestimmt}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
\textbf{Regel} & \textbf{Se} & \textbf{FP/Std} & \textbf{Se} & \textbf{FP/Std} \\
\midrule
1-von-1  & 40,4\,\% & 212,4 & 100,0\,\% & 280,8 \\
2-von-3  & 29,8\,\% & 100,0 &  97,9\,\% & 124,6 \\
3-von-5  & 17,0\,\% &  63,2 &  93,6\,\% &  73,7 \\
\textbf{5-von-7} & \textbf{8,5\,\%} & \textbf{29,8} & \textbf{83,0\,\%} & \textbf{21,1} \\
7-von-10 &  6,4\,\% &  10,5 &  68,1\,\% &  10,5 \\
\bottomrule
\end{tabularx}
\end{table}

Für das feinabgestimmte Modell reduziert die Regel \emph{5-von-7} die
Fehlalarmrate von $1192$ auf $21{,}1$ pro Stunde -- eine Verringerung
um $98\,\%$ -- bei einem Rückgang der Sensitivität von $100\,\%$ auf
$83{,}0\,\%$. Dies stellt für den vorgesehenen Einsatzzweck einen
vertretbaren Kompromiss dar.

\subsubsection{Voting ersetzt keine Domänenanpassung}

Der Vergleich beider Modellspalten in Tabelle~\ref{tab:voting} führt
zu einem Befund, der über die reine Parameterwahl hinausgeht. Beim
Übergang von \emph{1-von-1} zu \emph{5-von-7} verliert das
Basismodell $78\,\%$ seiner Sensitivität ($40{,}4\,\%$ auf
$8{,}5\,\%$), das feinabgestimmte Modell hingegen nur $17\,\%$
($100\,\%$ auf $83{,}0\,\%$).

Die Erklärung liegt in der zeitlichen Struktur der Ausgaben. Ein
nicht angepasstes Modell erzeugt auf fremden Daten \emph{vereinzelte},
zeitlich unzusammenhängende Positivmeldungen; diese werden durch die
Voting-Regel systematisch verworfen. Ein domänenangepasstes Modell
erzeugt dagegen einen \emph{zusammenhängenden} Block positiver
Fenster um das Aufprallereignis, der die Regel zuverlässig passiert.

Die zeitliche Plausibilisierung ist damit kein Ersatz für die
Domänenanpassung, sondern setzt diese voraus. Für die Firmware ergibt
sich daraus eine klare Reihenfolge: zuerst Fine-Tuning auf
Fußdaten, danach Voting zur Unterdrückung der verbleibenden
Fehlalarme.

\subsection{Hard-Negative-Training}
\label{sec:hardneg}

Da die Fehlalarme überwiegend bei Aktivitäten mit wiederholtem
Bodenkontakt auftreten, lag die Vermutung nahe, diese Fenster im
Training gezielt höher zu gewichten. Getestet wurde eine Gewichtung
mit Faktor~$3{,}0$ für alle ADL-Fenster mit einer
Spitzenbeschleunigung oberhalb von $1{,}8$\,g -- dies betraf $1401$
Fenster beziehungsweise $32{,}9\,\%$ aller ADL-Fenster.

\begin{table}[H]
\centering
\caption{Wirkung der Hard-Negative-Gewichtung
         (Regel 2-von-3, Schwelle $0{,}9$).}
\label{tab:hardneg}
\small
\begin{tabularx}{0.85\textwidth}{X C{2.2cm} C{2.2cm}}
\toprule
\textbf{Variante} & \textbf{Sensit.} & \textbf{FP/Std} \\
\midrule
Basismodell (SisFall, ohne Anpassung) &  6,4\,\% & 10,4 \\
Fine-Tuning ohne Gewichtung           & 72,3\,\% & 13,9 \\
Fine-Tuning mit Hard Negatives        & 63,8\,\% & 13,9 \\
\bottomrule
\end{tabularx}
\end{table}

Die Gewichtung erzielte nicht die erhoffte Wirkung: Die
Fehlalarmrate blieb unverändert, während die Sensitivität um
$8{,}5$~Prozentpunkte zurückging. Der Ansatz wird daher nicht
weiterverfolgt.

Aufschlussreich ist gleichwohl der Vergleich der ersten beiden
Zeilen: Das schlichte Fine-Tuning hebt die Sensitivität von
$6{,}4\,\%$ auf $72{,}3\,\%$. Die Domänenanpassung selbst ist damit
um ein Vielfaches wirksamer als jede der hier untersuchten
Zusatzmaßnahmen.

\subsection{Interpretation}

Die Kombination der Experimente -- einschließlich des FallAllD-Versuchs
und des zugehörigen Clipping-Kontrollexperiments -- erlaubt eine
differenziertere Aussage, als sie aus einem einzelnen Transferversuch
ableitbar gewesen wäre.

\textbf{Erstens gelingt der Transfer zwischen qualitativ hochwertigen
Datensätzen gut -- aber nur bei ähnlicher Sensorhardware.} Auf KFall
erreicht das unveränderte SisFall-Modell ohne jede Anpassung eine
Sensitivität von $83{,}5\,\%$ bei einer Spezifität von $94{,}9\,\%$ --
und das, obwohl sich Probandenkollektiv (Südkorea statt Kolumbien),
Sensorhardware (Xsens MTw~Awinda statt ADXL345/ITG3200) und
Aufnahmeprotokoll vollständig unterscheiden. FallAllD hingegen erreicht
bei noch höherer Abtastrate ($238$\,Hz) nur $13{,}6\,\%$. Das
Clipping-Kontrollexperiment (Abschnitt~\ref{sec:clipping}) schließt
den Messbereich ($\pm 8$\,g) als Ursache aus. Der entscheidende
Unterschied liegt daher in den sensorspezifischen
Signalcharakteristiken des LSM9DS1 gegenüber dem ADXL345/ITG3200.

\textbf{Zweitens ist die native Abtastrate ein notwendiger, aber kein
hinreichender Faktor.} UMAFall zeigt, dass eine zu niedrige native
Abtastrate ($20$\,Hz) den Transfer verhindert. FallAllD zeigt
umgekehrt, dass eine hohe Abtastrate allein ihn nicht garantiert.
Beide Ergebnisse zusammen ergeben: Guter Zero-Shot-Transfer erfordert
sowohl eine ausreichende Abtastrate \emph{als auch} eine hinreichend
ähnliche Signalcharakteristik der Hardware.

\textbf{Drittens bestätigt sich die Abtastratenempfehlung aus
Kapitel~\ref{ch:properties}, jedoch mit einer wichtigen Präzisierung.}
Eine Reduktion von $50$\,Hz auf $20$\,Hz kostet bei korrektem
Heruntertasten lediglich rund einen Prozentpunkt Sensitivität
($98{,}0\,\%$ auf $96{,}8\,\%$). Die Empfehlung gilt jedoch
\emph{nur} für gefiltertes Heruntertasten, nicht für eine native
Erfassung mit $20$\,Hz.

\textbf{Viertens ist der reine Positionseffekt moderat.} Innerhalb
von UMAFall, also bei identischer Hardware und identischem Protokoll,
kostet der Wechsel von der Taille zum Knöchel im Zero-Shot-Fall
$7{,}4$~Prozentpunkte ($42{,}2\,\%$ auf $34{,}8\,\%$) und nach dem
Fine-Tuning $12{,}1$~Prozentpunkte ($92{,}2\,\%$ auf $80{,}1\,\%$).
Die Sensorposition ist damit ein realer, aber nicht der dominierende
Faktor. Für das Vorhaben ist dies ein günstiger Befund, da er die
grundsätzliche Machbarkeit der fußnahen Anbringung stützt.

\textbf{Fünftens ist Transfer Learning in allen Konfigurationen
wirksam}, wobei der erforderliche Umfang mit der Ausgangsqualität
skaliert: Auf KFall genügt eine Verbesserung um $12{,}9$~Prozentpunkte
($83{,}5\,\%$ auf $96{,}4\,\%$), auf FallAllD sind es
$71{,}0$~Prozentpunkte ($13{,}6\,\%$ auf $84{,}6\,\%$). In keiner
Konfiguration verschlechterte sich dabei die Spezifität.

\textbf{Sechstens ist die Falsch-Positiv-Rate bei fußnaher Anbringung
das kritischere Problem als die Sensitivität.} Der Test auf
fußnahen ADL-Daten (Abschnitt~\ref{sec:fptest}) ergibt ohne Anpassung
$3{,}29\,\%$ Fehlalarme, konzentriert auf Aktivitäten mit
wiederholtem Bodenkontakt (Joggen: $17{,}76\,\%$, Gehen:
$5{,}33\,\%$). Da das Modell diese Fehlklassifikationen mit maximaler
Konfidenz ($p = 1{,}000$) trifft, greift eine reine Anhebung der
Entscheidungsschwelle zu kurz. Erforderlich sind stattdessen
domänenspezifische Trainingsdaten sowie ergänzende Mechanismen wie
eine zeitliche Mehrheitsentscheidung über mehrere aufeinanderfolgende
Fenster.

\textbf{Siebtens ist die Reihenfolge der Anpassungsschritte
entscheidend.} Das progressive Fine-Tuning
(Abschnitt~\ref{sec:progressive}) übertrifft die direkte Anpassung um
$41{,}7$~Prozentpunkte Sensitivität bei gleichzeitig höherer
Spezifität. Bemerkenswert ist, dass die Zwischenstufe für sich
genommen die Leistung \emph{verschlechtert} ($30{,}6\,\%$ auf
$22{,}2\,\%$) und ihr Nutzen sich erst im nachfolgenden
Anpassungsschritt zeigt. Der Zwischenschritt wirkt demnach nicht als
Leistungsverbesserung, sondern als Verallgemeinerung der gelernten
Merkmalsrepräsentation.

\textbf{Achtens ergänzen sich zeitliche Plausibilisierung und
Domänenanpassung, ersetzen einander aber nicht.} Die
N-von-M-Regel \emph{5-von-7} senkt die Fehlalarmrate des
angepassten Modells um $98\,\%$ bei einem Sensitivitätsverlust von
$17$~Prozentpunkten (Abschnitt~\ref{sec:voting}). Auf das nicht
angepasste Modell angewendet, vernichtet dieselbe Regel jedoch
$78\,\%$ der Sensitivität. Der Grund liegt in der zeitlichen
Struktur der Modellausgaben: Nur ein domänenangepasstes Modell
erzeugt um den Aufprall herum einen zusammenhängenden Block
positiver Fenster, den die Regel passieren lässt. Für die Umsetzung
folgt daraus eine feste Reihenfolge -- erst Anpassung, dann Voting.

Ergänzend ist festzuhalten, dass eine gezielte Höhergewichtung
impactstarker ADL-Fenster (Hard Negatives,
Abschnitt~\ref{sec:hardneg}) keine Verbesserung erbrachte. Die
Domänenanpassung selbst erwies sich in allen Versuchen als die mit
Abstand wirksamste Einzelmaßnahme.

\subsection{Konsequenzen für den Prototyp}
\label{sec:konsequenzen}

Aus den Experimenten ergeben sich zwei unmittelbar umsetzbare
Vorgaben für die Firmware des ESP32-C6.

\textbf{Erfassung mit hoher Rate und anschließendes Heruntertasten.}
Der Vergleich der beiden $20$-Hz-Bedingungen zeigt, dass die
\emph{Art} der Ratenreduktion entscheidend ist. Der MPU-6050 ist
daher mit einer Ausgangsdatenrate von mindestens $100$\,Hz zu
betreiben; das integrierte digitale Tiefpassfilter (DLPF) ist zu
aktivieren, und die Reduktion auf die Modelleingangsrate
($20$--$50$\,Hz) erfolgt anschließend softwareseitig. Eine direkte
Konfiguration des Sensors auf $20$\,Hz ist zu vermeiden, da sie
denselben Informationsverlust verursachen würde, der bei UMAFall
beobachtet wurde.

\textbf{Erhebung eines Pilotdatensatzes ist zwingend erforderlich.}
Das FallAllD-Experiment verdeutlicht, dass selbst bei idealer
Abtastrate und vergleichbarer Sensorposition ein Hardwarewechsel
den Zero-Shot-Transfer auf unter $14\,\%$ Sensitivität drücken kann.
Fine-Tuning hebt die Leistung zuverlässig an -- auf KFall auf
$96{,}4\,\%$, auf FallAllD auf $84{,}6\,\%$ --, setzt aber
Trainingsdaten der Zielhardware voraus. Da der MPU-6050 eine weitere,
gegenüber allen betrachteten Datensätzen abweichende Sensorhardware
\emph{und} eine abweichende Sensorposition darstellt, ist die
Anpassung an domänenspezifische Daten nicht optional, sondern
Voraussetzung für ein einsatzfähiges System.

\textbf{Konfiguration des Messbereichs.} Das Clipping-Experiment
zeigt, dass $\pm 8$\,g für die Sturzerkennung ausreichend sind; ein
$\pm 16$\,g-Bereich bringt keinen messbaren Vorteil. Der MPU-6050
sollte dennoch auf $\pm 16$\,g konfiguriert werden, um
Headroom für extreme Stürze zu gewährleisten und die
Signalcharakteristik der SisFall-Trainingsdaten bestmöglich zu
approximieren.

\textbf{Trainingsstrategie: progressiv statt direkt.} Aus
Abschnitt~\ref{sec:progressive} ergibt sich eine konkrete
Handlungsanweisung für die Anpassung an die Zielhardware. Das Modell
ist nicht unmittelbar von SisFall auf die Fußdaten anzupassen,
sondern über eine Zwischenstufe:
\[
\text{SisFall} \;\longrightarrow\; \text{KFall} \;\longrightarrow\;
\text{Pilotdaten (Fuß, MPU-6050)}
\]
Der Zwischenschritt kostet keinen zusätzlichen Aufwand bei der
Datenerhebung, da KFall bereits vorliegt, verbessert die
Anpassungsfähigkeit des Modells jedoch erheblich.

\textbf{Mehrstufige Alarmauslösung mit konkreten Parametern.} Aus
Abschnitt~\ref{sec:voting} ergibt sich ein unmittelbar umsetzbarer
Betriebspunkt für die Firmware:

\begin{table}[H]
\centering
\caption{Firmware-Vorgaben für die Alarmlogik.}
\label{tab:firmwarevorgabe}
\small
\begin{tabularx}{0.8\textwidth}{X l}
\toprule
\textbf{Parameter} & \textbf{Wert} \\
\midrule
Abtastrate des Sensors (ODR)      & $\geq 100$\,Hz, DLPF aktiv \\
Messbereich                        & $\pm 16$\,g \\
Modelleingang                      & $100 \times 6$ ($2$\,s @ $50$\,Hz) \\
Inferenzintervall                  & $200$\,ms \\
Ringpuffer für Entscheidungen      & 7 \\
Entscheidungsschwelle              & $0{,}9$ \\
Alarmbedingung                     & $\geq 5$ von 7 positiv \\
\bottomrule
\end{tabularx}
\end{table}

Bei einem Inferenzintervall von $200$\,ms und einer geschätzten
Inferenzzeit von etwa $10$\,ms ergibt sich eine Rechenlast von rund
$5\,\%$ -- der ESP32-C6 ist damit deutlich unterfordert, sodass
ausreichend Reserven für Funkübertragung und Energiesparmodi
verbleiben. Die konkrete Inferenzzeit ist nach der
Hardware-Integration zu verifizieren.

\section{Nächste Schritte}
\label{sec:nextsteps}

\subsection{Einordnung der durchgeführten Kontrollexperimente}

Die in den Abschnitten~\ref{sec:crossposition}
bis~\ref{sec:hardneg} berichteten Experimente umfassen drei
externe Datensätze (KFall, UMAFall, FallAllD), ein
Clipping-Kontrollexperiment, einen Falsch-Positiv-Test auf fußnahen
Alltagsdaten, einen Vergleich dreier Transferstrategien sowie zwei
Versuche zur Verringerung der Fehlalarmrate.

Insgesamt wurden fünf Hypothesen widerlegt -- die vermutete Dominanz
der Sensorposition, die alleinige Erklärungskraft der Abtastrate, der
Einfluss des Messbereichs, die Wirksamkeit einer
Hard-Negative-Gewichtung sowie die Annahme, eine
Einzelfensterentscheidung ließe sich unabhängig von der
Fensterschrittweite plausibilisieren. Bestätigt wurden vier: die
konsistente Wirksamkeit des Fine-Tunings, die dominierende Rolle
sensorspezifischer Signalcharakteristiken beim Domänentransfer, der
Vorteil eines progressiven gegenüber einem direkten
Anpassungsvorgehen und die Wirksamkeit der zeitlichen
Plausibilisierung -- letztere allerdings nur in Verbindung mit einem
domänenangepassten Modell.

\subsection{Verbleibende Arbeitsschritte}
...
\item \textbf{Pilotdatenerhebung am Fuß:} Aufzeichnung eines kleinen
      Fußdatensatzes (Gehen, Laufen, Treppen, Hinsetzen sowie
      simulierte Stürze auf Matte) zur Validierung und zum
      Fine-Tuning.
\item \textbf{Progressive Domänenanpassung:} Schrittweises
      Fine-Tuning des CNN: SisFall $\rightarrow$ KFall $\rightarrow$
      Pilotdaten, um die Domänenlücke zwischen den öffentlichen
      Datensätzen und der Zielhardware systematisch zu schließen.
\item \textbf{Verifikation der Alarmlogik auf Zielhardware:}
      Die in Tabelle~\ref{tab:firmwarevorgabe} festgelegten Parameter
      der Voting-Regel sind nach der Pilotdatenerhebung auf den
      tatsächlichen Fußdaten nachzujustieren, da sie bislang an
      UMAFall-Knöcheldaten bestimmt wurden.
