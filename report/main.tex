...
>>>>>>> b25c8ba4 (Add FallAllD transfer results + clipping experiment to Kapitel 5)
\bottomrule
\end{tabularx}
\end{table}
...
\times 0{,}000244$\,g, $\text{GYR} = \text{raw} \times 0{,}07$\,°/s).
...
<<<<<<< HEAD
Die Kombination der Experimente erlaubt eine Aussage, die aus einem
einzelnen Transferversuch nicht ableitbar gewesen wäre. Insbesondere
widerlegt das KFall-Experiment die zunächst naheliegende Vermutung,
der Datensatzwechsel als solcher verursache den Leistungsabfall.

\textbf{Erstens gelingt der Transfer zwischen qualitativ hochwertigen
Datensätzen gut.} Auf KFall erreicht das unveränderte SisFall-Modell
ohne jede Anpassung eine Sensitivität von $83{,}5\,\%$ bei einer
Spezifität von $94{,}9\,\%$ -- und das, obwohl sich Probandenkollektiv
(Südkorea statt Kolumbien), Sensorhardware (Xsens MTw~Awinda statt
ADXL345/ITG3200) und Aufnahmeprotokoll vollständig unterscheiden. Ein
Datensatzwechsel führt folglich \emph{nicht} zwangsläufig zu einem
Einbruch.

\textbf{Zweitens ist der Einbruch bei UMAFall auf Eigenschaften dieses
Datensatzes zurückzuführen.} Da der Transfer nach KFall gelingt, muss
die Ursache in den spezifischen Merkmalen von UMAFall liegen. Als
maßgeblich ist die native Abtastrate von $20$\,Hz anzusehen. Der
Vergleich der beiden $20$-Hz-Bedingungen ist hier aufschlussreich:
Ein aus $200$\,Hz auf $20$\,Hz \emph{heruntergetastetes} Signal
(SisFall) liefert weiterhin $96{,}8\,\%$ Sensitivität, während ein
\emph{nativ} mit $20$\,Hz aufgezeichnetes Signal (UMAFall) auf
$34{,}0\,\%$ fällt. Der Unterschied ist physikalisch erklärbar: Beim
Heruntertasten wird das Signal zuvor tiefpassgefiltert, sodass die
Einhüllende des Aufprallereignisses erhalten bleibt. Bei nativer
Aufzeichnung mit $20$\,Hz entfallen auf die typischerweise
$50$--$100$\,ms dauernde Aufprallphase hingegen nur ein bis zwei
Abtastwerte, sodass der Impact-Peak häufig gar nicht erfasst oder
durch Aliasing verfälscht wird. Hinzu kommt, dass UMAFall lediglich
drei Sturzarten umfasst (KFall: 15), was die Musterviel\-falt im
Zielbereich zusätzlich einschränkt.

\textbf{Drittens ist der reine Positionseffekt moderat.} Innerhalb
=======
...
<<<<<<< HEAD
\textbf{Viertens bestätigt sich die Abtastratenempfehlung aus
Kapitel~\ref{ch:properties}, jedoch mit einer wichtigen Präzisierung.}
Eine Reduktion von $50$\,Hz auf $20$\,Hz kostet bei korrektem
Heruntertasten lediglich rund einen Prozentpunkt Sensitivität
($98{,}0\,\%$ auf $96{,}8\,\%$). Die Empfehlung gilt jedoch
...
($83{,}5\,\%$ auf $96{,}4\,\%$), auf den UMAFall-Knöcheldaten sind es
$45{,}3$~Prozentpunkte ($34{,}8\,\%$ auf $80{,}1\,\%$). In keiner
=======
\textbf{Fünftens ist Transfer Learning in allen Konfigurationen
wirksam}, wobei der erforderliche Umfang mit der Ausgangsqualität
skaliert: Auf KFall genügt eine Verbesserung um $12{,}9$~Prozentpunkte
...
<<<<<<< HEAD
\textbf{Erhebung eines Pilotdatensatzes bleibt erforderlich.} Zwar
gelingt der Transfer zwischen hochwertigen Datensätzen besser als
zunächst angenommen, doch verbleibt auch im günstigsten Fall (KFall)
ein Rückstand von $14{,}5$~Prozentpunkten gegenüber der Referenz, der
erst durch Fine-Tuning geschlossen wird. Da die Zielhardware
zusätzlich eine gegenüber allen betrachteten Datensätzen abweichende
Sensorposition verwendet, ist eine Anpassung an domänenspezifische
Daten weiterhin vorzusehen.
=======
...
<<<<<<< HEAD
Die in Abschnitt~\ref{sec:crossposition} berichteten Experimente
wurden bereits durchgeführt und haben zwei Hypothesen widerlegt
(Positionsdominanz, Abtastraten-Störfaktor) sowie zwei bestätigt
(Wirksamkeit des Fine-Tunings, Eignung von $20$\,Hz). Offen bleibt,
welcher Anteil des verbleibenden Rückstands auf die spezifischen
Einschränkungen von UMAFall zurückgeht -- insbesondere auf die
Beschränkung auf drei Sturztypen und das Klassenungleichgewicht
($471$ zu $6329$ Fenster). Eine Kreuzvalidierung gegen KFall
\cite{kfall2021}, das mit 15~Sturztypen deutlich breiter angelegt
ist, ist hierfür vorgesehen.

\subsection{Weitere Arbeitsschritte}
=======
Die in den Abschnitten~\ref{sec:crossposition}
bis~\ref{sec:crossfallalld} berichteten Experimente umfassen drei
externe Datensätze (KFall, UMAFall, FallAllD) sowie ein
Clipping-Kontrollexperiment. Insgesamt wurden drei Hypothesen
widerlegt -- die vermutete Dominanz der Sensorposition, die alleinige
Erklärungskraft der Abtastrate und der Einfluss des Messbereichs --
sowie zwei bestätigt: die konsistente Wirksamkeit des Fine-Tunings
und die dominierende Rolle sensorspezifischer Signalcharakteristiken
beim Domänentransfer.
...
\item \textbf{False-Positive-Test auf Fußdaten:} Evaluierung des
      Modells auf der Smart Insole Database, die ausschließlich
      ADL-Aktivitäten vom Fuß enthält, um die Falsch-Positiv-Rate
      bei fußtypischen Bewegungsmustern zu bewerten.
